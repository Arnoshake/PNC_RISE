...
MOCK_SETTINGS: dict[str, CardSettings] = {
    "card-1": CardSettings(card_id="card-1", round_up_percentage=5.0, is_active=True),
    "card-2": CardSettings(card_id="card-2", round_up_percentage=5.0, is_active=True),
}


@router.get("/{card_id}", response_model=CardSettings)
def get_card_settings(card_id: str):
...
def update_card_settings(card_id: str, payload: CardSettingsUpdate):
